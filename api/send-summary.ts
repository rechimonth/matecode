...
type SummaryBody = { name?: unknown };
type FirestoreTask = { title?: unknown; status?: unknown; updatedAt?: unknown };
...
async function readJsonBody(req: ApiRequest): Promise<SummaryBody> {
  if (req.body !== undefined) {
    if (typeof req.body === "string") {
      if (!req.body.trim()) throw new Error("Empty body");
      return JSON.parse(req.body) as SummaryBody;
    }
    if (Buffer.isBuffer(req.body)) {
      const raw = req.body.toString("utf8");
      if (!raw.trim()) throw new Error("Empty body");
      return JSON.parse(raw) as SummaryBody;
    }
    if (typeof req.body === "object" && req.body !== null && !Array.isArray(req.body)) {
      return req.body as SummaryBody;
    }
